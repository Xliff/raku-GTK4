...
my $a = GTK::Application.new( 'org.genex.radiobutton' );

$a.Activate.tap(-> *@a {
  my @group = do for 1..3 {
    GTK::CheckButton.new-with-label("Radio Button $_");
  };
  .set-group( @group.head ) for @group.skip(1);

  for @group -> $r is copy {
    # It's better to use LEXICAL variable, not a loop varianle, in closures!
    $r.Toggled.tap(-> *@a { say "{ $r.label } selected." if $r.active })
  }
