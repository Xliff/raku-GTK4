...
sub add_button($s, $css) {
  my $i = GTK::Image.new_from_icon_name('image-missing');
  my $c = GDK::Cursor.new_from_name($css);
  with $c {
  ...
  $b.Clicked.tap( -> *@a {
    ...
    # say "B: { $b.getType }";
    # say "W0: { $.getType }";
    # say "A0: { GTK::ApplicationWindow.objType.getType }";
    $b.cursor = $c;
  });
  $b.tooltip_text = $css;
  $s.add($b);
...
$app = GTK::Application.new( title => 'org.genex.cursors' );
...
$app.Activate.tap( -> *@a {
  ...
  $sw.set-policy(GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
  ...
  for @h.grep( *.key ne 'Resize & Scrolling' ) -> $p {
    ...
    add_button(@sections.tail, $_) for $p.value.List;
  }
